...
/// Defines a command action that returns a [Result] of type [T].
/// Takes an arguments of type [A] and [B].
/// Used by [Command2] for actions with one argument.
...
final class Command2<T, A, B> extends Command<T> {
  ...
  /// Executes the action with the specified [argument] and [argument1].
  Future<void> execute(A argument, B argument1) async {
    await _execute(() => _action(argument, argument1));
  }
}